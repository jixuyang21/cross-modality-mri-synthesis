...
from .networks import CycleGenerator, CycleDiscriminator, weights_init_normal
...
class CycleGAN:
    def __init__(self, opt):
        ...
        self.generator_AB = CycleGenerator(1, opt.num_residual_blocks).to(self.device)
        self.generator_BA = CycleGenerator(1, opt.num_residual_blocks).to(self.device)
        self.discriminator_A = CycleDiscriminator(1).to(self.device)
        self.discriminator_B = CycleDiscriminator(1).to(self.device)
        self.init_weight()
    ...
    def compute_loss_G(self):
        ...
        loss_G_B_id = self.loss_l1(self.fake_B, self.real_B)
        loss_G_B_cyc = self.loss_l1(self.rec_B, self.real_B)
        
        loss_G_A_adv = self.loss_gan(pred_fake_A, valid)
        loss_G_A_id = self.loss_l1(self.fake_A, self.real_A)
        loss_G_A_cyc = self.loss_l1(self.rec_A, self.real_A)
        
        self.loss_G = self.opt.lambda_adv * (loss_G_A_adv + loss_G_B_adv) / 2 + self.opt.lambda_id * (loss_G_A_id + loss_G_B_id) / 2 + self.opt.lambda_cyc * (loss_G_A_cyc + loss_G_B_cyc) / 2
